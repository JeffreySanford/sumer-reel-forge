...
function productionSourceRoot(): string {
  const workspacePath = resolve(process.cwd(), 'libs/animation-compiler/src/lib');
  if (existsSync(workspacePath)) return workspacePath;
  return resolve(process.cwd(), 'src/lib');
}

describe('Scene V3 compiler determinism policy', () => {
  it('contains no global randomness or wall-clock semantic state', async () => {
