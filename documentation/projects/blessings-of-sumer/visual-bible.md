# Blessings Of Sumer Visual Bible

## Status

- Version: `v1`
- Baseline asset: `assets/visual-bible/blessings-of-sumer-master-v1.png`
- Approval state: production baseline for the Reel 1 editorial cut
- Review scope: character identity, vessel design, environment, material palette, respectful cultural presentation, and motion continuity

## Visual Direction

Use cinematic mythic realism grounded in southern Mesopotamian materials and landscapes. Images should feel tactile and lived-in rather than like polished high fantasy.

- Format reel artwork at `9:16`, with important faces and action inside the central safe area.
- Favor water, reeds, bitumen-dark timber, woven reed structures, clay, wool, copper, lapis, and restrained gold accents.
- Use natural dawn and late-afternoon light, visible atmosphere, realistic skin, and detailed practical clothing.
- Avoid Egyptian crowns, Greco-Roman architecture, medieval armor, modern objects, ornamental fantasy weapons, and text generated inside images.
- Keep divine imagery numinous but physical. Supernatural elements should emerge from water, light, wind, or scale rather than spectacle-heavy effects.

## Continuity Anchors

### Enki

- Mature Mesopotamian man with warm brown skin, long black wavy hair, and a full dark beard with subtle gray.
- Cream woven robe with narrow lapis-blue and copper trim, broad copper belt, lapis and copper jewelry, leather sandals.
- Calm, observant authority. He is a traveler and builder in Reel 1, not a warrior.
- The compact horned divine crown from the baseline remains the identity anchor. Keep it restrained in action shots and do not enlarge it into an Egyptian or theatrical silhouette.

### Stag Of The Absu

- Working reed-and-dark-timber sailing vessel with high curved prow and stern.
- Woven shelter, rigging, clay jars, timber bundles, reed bundles, tools, and modest livestock cargo.
- It must read as a practical expedition vessel, not a Viking longship, galleon, or royal barge.

### Nammu

- Primordial feminine presence formed from deep blue water, foam, suspended light, and current.
- Serene and ancient, never horror-coded or rendered as a conventional mermaid.
- She can remain implied rather than fully visible when the narration treats the sea as memory or origin.

### Dilmun

- Luminous low coast with pale stone, clear water, reeds, birds, palms in the distance, and an atmosphere of abundance.
- Distinct from the darker open water at the beginning of the voyage.
- Avoid monumental fantasy cities in the Reel 1 approach shots.

### People And Daily Life

- Families, travelers, and laborers wear practical woven wool or linen garments in mineral and earth colors.
- Show varied ages and individual faces. Treat Martu and other communities as people with agency, skills, families, and material culture.
- Do not use visual shorthand based on dirt, menace, or caricature to signal cultural difference.

## Color And Composition

- Deep water: `#082f42`
- Lapis: `#1d5f82`
- Copper: `#a6653a`
- Reed gold: `#c9a65b`
- Clay: `#9a6047`
- Limestone: `#d8c9aa`
- Woven cream: `#e5d8bd`
- Near-black bitumen: `#171918`

The opening should progress from cool black-blue water toward warm Dilmun light. Compose for motion crops and captions: keep the lower 20 percent and outer 8 percent free of critical faces or story details.

## Motion Extension

The visual baseline remains authoritative when artwork is adapted for animation.

Use `planning/reel-01-animation-style-bible.md` for the detailed motion grammar. The following principles are continuity rules for all Blessings of Sumer animation:

- Camera movement should usually create more energy than character movement.
- Character motion should remain restrained: breathing, blink, gaze, slight head turn, cloth response, and occasional deliberate gesture.
- Foreground motion and atmospheric layers should create depth without distracting from faces or symbolic objects.
- Environmental motion should not share one obvious repeated cycle across water, cloth, reeds, rigging, and characters.
- Supernatural movement should emerge through water, refraction, light, scale, and atmosphere rather than conventional fantasy effects.
- Nammu should feel primordial and environmental rather than monster-like or mermaid-like.
- Lighting should move from cool open water toward warmer Dilmun scenes while staying materially plausible.
- Lip sync is optional and should only be used for visible on-camera speech, not automatically under third-person narration.
- Layered animation assets must remain consistent with approved character, vessel, environment, and palette references.

Animation quality is reviewed separately from technical render success. A technically valid MP4 is not automatically an approved visual asset.

## Layered Animation Asset Policy

The eight approved `editorial-v1` frames remain source references and must not be overwritten.

Animation-specific derived assets should use versioned directories such as:

```text
assets/blessings-of-sumer/chapter-01/reel-01/animation-v1/shot-03/
```

Where practical, separate:

- distant background;
- midground environment;
- water/terrain;
- vessel or major prop;
- character body/head;
- eye/blink states;
- optional cloth/hair overlays;
- foreground occluders;
- atmosphere;
- reflection/light masks.

Derived layers should preserve provenance back to the approved editorial frame or regenerated visual-bible-consistent source.

## Naming And Source Consistency

- Use `Uruk (Erech)` on first audience-facing mention, then `Uruk`.
- Keep `Eridu` distinct from Uruk; they are separate cities in the story world.
- Use `E-Absu` as the canonical temple name. `E-engura` may appear as an epithet only when the source calls for it.
- Keep Ninsikila, Nintu, and Ninhursag as separate named narrative figures or roles unless a source-specific editorial note explicitly identifies them.
- Record any deliberate modernization, composite identity, or disputed translation in the chapter production notes before rendering.

## Reel 1 Shot Baseline

1. Black water before dawn with a faint gold horizon and low mist.
2. The Stag under sail with expedition cargo visible.
3. Enki at the helm, calm and purposeful.
4. Nammu implied beneath the deep water as origin and memory.
5. A future traveler shrine with spring water, oven, bread, and welcome.
6. A symbolic practical montage of basin, bread, tablet, scales, and open road.
7. Dilmun appearing through haze with reeds and birds.
8. The Stag approaching land with open composition for the final title treatment.

Shot 3 is the primary Sprint 006 animation-quality benchmark. Shot 4 or Shot 7 should be used as the second contrasting benchmark before the animation language is propagated across the remaining shots.

## Approval Checklist

- [x] Enki identity and clothing baseline established.
- [x] Stag vessel and cargo baseline established.
- [x] Nammu visual language established.
- [x] Dilmun environment baseline established.
- [x] Respectful community representation baseline established.
- [x] Reel 1 animation motion and layered-asset policy documented.
- [ ] Review the eight generated Reel 1 frames as a continuous sequence.
- [ ] Approve the Shot 3 layered animation benchmark against the animation style bible.
- [ ] Approve a second contrasting benchmark before scaling the animation language.
- [ ] Record requested continuity changes as `v2` rather than silently replacing the approved baseline.
