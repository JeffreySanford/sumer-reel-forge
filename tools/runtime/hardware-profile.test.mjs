...
function profile({
  ...
  return {
    ...
    gpu: {
      ...
      devices: vramMb
        ? [{ vendor: nvidia ? 'NVIDIA' : 'AMD', name: 'Test GPU', memoryTotalMb: vramMb }]
        : [],
    },
    media: {
...
test('constrained profile scales down without assuming a GPU', () => {
  const input = profile({
    cpu: 4,
    memory: 8,
    vramMb: 0,
...
test('startup environment application never overwrites explicit settings', () => {
  ...
  applyHardwareProfileEnvironment(input, env, 'tmp/runtime/hardware-profile.json');
  ...
  assert.equal(env.SRF_HARDWARE_PROFILE_PATH, 'tmp/runtime/hardware-profile.json');
});
