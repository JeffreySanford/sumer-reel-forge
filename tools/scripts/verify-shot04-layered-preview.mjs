...
const EXPECTED_DURATION_SECONDS = 8;
...
assertInside(PREVIEW_ROOT, previewDirectory, 'Shot 4 preview directory');

const manifestPath = join(previewDirectory, 'preview-manifest.json');
if (!existsSync(manifestPath)) {
  throw new Error(`Shot 4 layered preview manifest not found: ${manifestPath}`);
}
const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
if (manifest.previewType !== 'shot04-layered-candidate-preview') {
  throw new Error(
    `Expected shot04-layered-candidate-preview, found ${manifest.previewType ?? 'unknown'}.`,
  );
}
if (!Array.isArray(manifest.candidates) || manifest.candidates.length !== 4) {
  throw new Error('Shot 4 layered preview must record exactly four required candidates.');
}
...
const videoPath = resolvePathInside(
  ...
  'Shot 4 preview video',
);
if (!videoPath || !existsSync(videoPath)) {
  throw new Error(`Shot 4 layered preview video not found: ${videoPath}`);
}

const media = probeVideo(videoPath);
const videoStream = media.streams.find((stream) => stream.codec_type === 'video');
if (!videoStream) throw new Error('Shot 4 layered preview MP4 has no video stream.');
...
if (reviewFrames.length < requiredMarkers.length) {
  throw new Error('Shot 4 preview must contain all five Nammu review markers.');
}
const frames = Object.fromEntries(
  requiredMarkers.map((id, index) => [id, findReviewFrame(reviewFrames, id, index)]),
);
const comparisons = [
  measureFrameDifference(
    frames['physical-water'].path,
    frames['unusual-pattern'].path,
    'physical-water→unusual-pattern',
  ),
  measureFrameDifference(
    frames['unusual-pattern'].path,
    frames.recognition.path,
    'unusual-pattern→recognition',
  ),
  measureFrameDifference(
    frames.recognition.path,
    frames['coherence-peak'].path,
    'recognition→coherence-peak',
  ),
  measureFrameDifference(
    frames['coherence-peak'].path,
    frames.dissolution.path,
    'coherence-peak→dissolution',
  ),
];
...
const report = {
  schemaVersion: 1,
  verificationType: 'shot04-layered-environmental-coherence-motion',
  ...
  motion: {
    ...
    comparisons,
    interpretation:
      'Passing proves the assembled environmental candidate changes through the Nammu recognition arc. Human review must still confirm that the change reads as water/refraction/coherence rather than a literal character fade or generic fantasy effect.',
  },
  humanReview: {
    required: true,
    questions: [
      'Does the opening still read as physical water rather than an effect shot?',
      'Does recognition emerge from current/refraction structure rather than a conventional character entrance?',
      'At coherence peak, is Nammu perceptible without becoming a hard cutout, mermaid, aura, or glowing-eyed figure?',
      'Does dissolution return attention to water rather than feeling like a disappearance effect?',
      'Does the camera remain effectively still while the environment carries the reveal?',
    ],
  },
};
...
console.log('Shot 4 layered candidate verification');
console.log(`Preview: ${previewDirectory}`);
console.log('[ok] upstream QA: deep water + mid-current + surface refraction + Nammu coherence');
...
console.log(`Environmental motion QA: ${motionPass ? 'PASS' : 'FAIL'}`);
...
function newestPreviewDirectory() {
  if (!existsSync(PREVIEW_ROOT)) {
    throw new Error(
      `Shot 4 preview root does not exist: ${PREVIEW_ROOT}. Run the Shot 4 candidate preview first.`,
    );
  }
  ...
  if (!directories.length) throw new Error(`No Shot 4 previews found under ${PREVIEW_ROOT}.`);
