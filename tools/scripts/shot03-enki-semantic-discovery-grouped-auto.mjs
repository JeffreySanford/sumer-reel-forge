...
const locator = normalizePixelBox(located.bboxPixels, source);
const locatorBottom = locator.y + locator.height;
const lowerExtension = source.height - locatorBottom;
if (lowerExtension < 0) throw new Error('Recorded Enki locator extends beyond source bottom.');
const crop = {
  x: locator.x,
  y: locator.y,
  width: locator.width,
  height: source.height - locator.y,
};
const cropAreaShare = (crop.width * crop.height) / (source.width * source.height);
if (cropAreaShare >= 0.9) {
  throw new Error(`Grouped Enki semantic crop covers ${(cropAreaShare * 100).toFixed(2)}% of frame; refusing near-full-frame proxy.`);
}
...
const proxyPath = join(discoveryDirectory, 'semantic-grouped-locator-crop.png');
...
const metadata = {
  ...
  semanticMode: 'grouped-face-body-hands',
  source,
  crop,
  cropAreaShare,
  originalLocator: locator,
  lowerBoundaryExtensionPixels: lowerExtension,
  lowerBoundaryExtensionReason:
    'Prior exact-locator semantic runs repeatedly returned found hand/arm/torso boxes with y+height beyond 1; extend only to immutable source bottom before changing model task shape.',
  ...
  visionGuideRole: 'immutable editorial source from original accepted ROI-search receipt',
  ...
  paddingFraction: 0,
  sideOrTopExpansionPixels: 0,
  priorAttemptArchive: archivedAttempt,
  sourcePixelsMutated: false,
  generatedSemanticPixels: false,
  samInvocations: 0,
  modelLocatorInvocations: 0,
  semanticGroups: ['face-head', 'body-arms', 'hands-contact'],
  coordinatePolicy:
    'reuse recorded Enki locator x/y/width, extend only lower boundary to immutable source bottom, run smaller semantic groups, remap crop-normalized geometry to registered source frame',
};
writeFileSync(metadataPath, `${JSON.stringify(metadata, null, 2)}\n`, 'utf8');

console.log('Shot 3 Enki grouped semantic discovery autopilot');
console.log('Policy: split the 19-item task into face/head, body/arms, and hands/contact groups after the one-shot full-task repair failed.');
console.log(`Workspace: ${workspace}`);
if (archivedAttempt) console.log(`Archived prior discovery evidence: ${archivedAttempt}`);
console.log(`Original locator: ${JSON.stringify(locator)} · confidence ${Math.round(Number(located.confidence ?? 0) * 100)}%`);
console.log(`Lower-boundary extension: ${lowerExtension}px; side/top expansion: 0px.`);
...
const hook = pathToFileURL(resolve('tools/scripts/enki-semantic-grouped-vision-hook.mjs')).href;
...
function normalizePixelBox(box, sourceSize) {
  ...
  if (x + width > sourceSize.width || y + height > sourceSize.height) {
    throw new Error('Recorded Enki locator exceeds actor-prep source registration.');
  }
...
function archiveExistingDiscovery(actorWorkspace) {
  ...
  const archive = join(historyRoot, `${stamp}-pre-grouped-semantic`);
...
function renderExactCrop(inputPath, outputPath, box) {
  ...
  if (result.status !== 0 || !existsSync(outputPath)) {
    throw new Error(`FFmpeg grouped semantic crop failed: ${result.stderr || `exit ${result.status}`}`);
  }
}
