...
assertInside(PREVIEW_ROOT, previewDirectory, 'Preview directory');

const manifestPath = join(previewDirectory, 'preview-manifest.json');
if (!existsSync(manifestPath)) {
  throw new Error(`Preview manifest not found: ${manifestPath}`);
}

const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'));
const videoPath = resolvePathInside(
  previewDirectory,
  manifest?.output?.path ?? join(previewDirectory, 'shot03-water-candidate-preview.mp4'),
  'Preview video',
);
if (!existsSync(videoPath)) {
  throw new Error(`Preview video not found: ${videoPath}`);
}
...
const metadataChecks = [
  ...
  },
  {
    ...
    pass:
      ...
      Math.abs(durationSeconds - EXPECTED_DURATION_SECONDS) < 0.2,
...
if (reviewFrames.length < 3) {
  throw new Error(
    `Expected at least 3 review frames in ${manifestPath}; found ${reviewFrames.length}.`,
  );
}
...
const motionPass = comparisons.every(
  (comparison) =>
    comparison.meanAbsoluteDifference >= options.minMeanDiff &&
    comparison.changedPixelRatio >= options.minChangedRatio,
);
...
const report = {
  ...
  motion: {
    ...
    note:
      'Motion is measured from decoded review-frame pixels above the bottom review-guide region; this proves rendered pixels changed, not that the motion is artistically correct.',
    comparisons,
  },
};
...
console.log('Shot 3 water rendered-motion verification');
...
if (!pass) {
  console.error(
    'Rendered animation verification failed. Do not treat this preview as proof of working motion.',
  );
  process.exitCode = 2;
}
...
function measureFrameDifference(firstPath, secondPath, label) {
  ...
  const filter =
    `[0:v]crop=${EXPECTED_WIDTH}:${cropHeight}:0:0[a];` +
    `[1:v]crop=${EXPECTED_WIDTH}:${cropHeight}:0:0[b];` +
...
function newestPreviewDirectory() {
  if (!existsSync(PREVIEW_ROOT)) {
    throw new Error(
      `Preview root does not exist: ${PREVIEW_ROOT}. Run pnpm comfyui:water:preview first.`,
    );
  }
  ...
  if (!directories.length) {
    throw new Error(`No preview directories found under ${PREVIEW_ROOT}.`);
  }
...
function parseOptions(args) {
  ...
  for (const arg of args) {
    if (arg.startsWith('--preview-dir=')) {
      ...
      options.minMeanDiff = positiveNumber(arg, '--min-mean-diff=');
    } else if (arg.startsWith('--min-changed-ratio=')) {
      options.minChangedRatio = positiveNumber(arg, '--min-changed-ratio=');
    } else if (arg.startsWith('--pixel-change-threshold=')) {
      options.pixelChangeThreshold = positiveNumber(
        arg,
        '--pixel-change-threshold=',
      );
    } else if (arg.startsWith('--ignore-bottom-pixels=')) {
      options.ignoreBottomPixels = positiveNumber(
        arg,
        '--ignore-bottom-pixels=',
      );
    } else {
...
function positiveNumber(arg, prefix) {
