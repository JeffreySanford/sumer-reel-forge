...
const PREVIEW_ROOT = resolve('tmp/animation-previews/shot03-water-preview');
...
async function main(): Promise<void> {
  ...
  const candidate = await resolveWaterCandidate(options.candidateDir);
  ...
  const stagedSourceName = 'shot03-editorial-source.png';
  const stagedWaterName = 'shot03-water-candidate.png';
  const stagedSource = join(publicDirectory, stagedSourceName);
  const stagedWater = join(publicDirectory, stagedWaterName);
  await Promise.all([
    copyFile(SOURCE_PATH, stagedSource),
    copyFile(candidate.path, stagedWater),
  ]);

  const props = {
    sourceAsset: stagedSourceName,
    waterAsset: stagedWaterName,
  ...
  const videoPath = join(outputDirectory, 'shot03-water-candidate-preview.mp4');
  ...
  console.log('Rendering Shot 3 water candidate preview...');
  ...
  await writeJson(manifestPath, {
    ...
    motion: {
      preset: 'water-candidate-drift-v1',
      strength: options.motionStrength,
      purpose:
        'Expose segmentation quality with restrained independent water motion before human approval.',
    },
    renderProfile,
    renderDurationMs,
    output: {
...
async function resolveWaterCandidate(
  ...
  for (const runDirectory of runDirectories) {
    ...
    try {
      manifest = JSON.parse(await readFile(manifestPath, 'utf8')) as CandidateRunManifest;
    } catch {
      continue;
    }

    const fromManifest = await candidatePathFromManifest(runDirectory, manifest);
    if (fromManifest) {
      return { runDirectory, path: fromManifest, manifest };
    }

    const pngs = await collectPngFiles(runDirectory);
    const named = pngs.filter((path) => {
      const name = basename(path).toLowerCase();
      return name.includes(LAYER_ID) || name.includes('water');
    });
    const fallback = named.length === 1 ? named[0] : pngs.length === 1 ? pngs[0] : undefined;
    if (fallback) {
      assertInside(runDirectory, fallback, 'Candidate PNG');
      return { runDirectory, path: fallback, manifest };
    }
  }

  throw new Error(
    `No ${LAYER_ID} PNG was found in the selected candidate run(s).`,
  );
}
...
async function candidatePathFromManifest(
  runDirectory: string,
  manifest: CandidateRunManifest,
  ...
  for (const entry of manifest.candidates ?? []) {
    ...
    if (layerId !== LAYER_ID && !rawPath?.toLowerCase().includes('water')) {
      continue;
    }
    if (!rawPath) continue;
    ...
    for (const path of candidates) {
      if (!isInside(runDirectory, path)) continue;
...
function readPngDimensions(buffer: Buffer, label: string): {
  width: number;
  height: number;
} {
  if (
