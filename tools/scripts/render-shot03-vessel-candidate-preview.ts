...
const LAYER_ID = 'shot03-vessel-v1';
const CANDIDATE_ROOT = resolve(
  'tmp/animation-assets/candidates/chapter-01-reel-01-animation-v1',
);
const PREVIEW_ROOT = resolve('tmp/animation-previews/shot03-vessel-preview');
...
async function main(): Promise<void> {
  ...
  const candidate = await resolveVesselCandidate(options.candidateDir);
  ...
  const vesselName = 'shot03-vessel-candidate.png';
  await Promise.all([
    copyFile(SOURCE_PATH, join(publicDirectory, sourceName)),
    copyFile(candidate.path, join(publicDirectory, vesselName)),
  ]);

  const props = {
    sourceAsset: sourceName,
    vesselAsset: vesselName,
  ...
  const videoPath = join(outputDirectory, 'shot03-vessel-candidate-preview.mp4');
  ...
  console.log('Rendering Shot 3 vessel candidate preview...');
  ...
  await writeJson(manifestPath, {
    ...
    motion: {
      preset: 'heavy-vessel-diagnostic-v1',
      strength: options.motionStrength,
      purpose:
        'Expose vessel segmentation quality using slow rigid-body inertia before human approval.',
    },
    renderProfile,
    renderDurationMs,
    output: {
...
async function resolveVesselCandidate(configuredDirectory?: string): Promise<{
  runDirectory: string;
  path: string;
  ...
  for (const runDirectory of runDirectories) {
    ...
    for (const entry of manifest.candidates ?? []) {
      ...
      if (layerId !== LAYER_ID && !rawPath?.toLowerCase().includes('vessel')) continue;
      if (!rawPath) continue;
    ...
    const vessel = pngs.filter((path) => {
      const name = basename(path).toLowerCase();
      return name.includes(LAYER_ID) || name.includes('vessel');
    });
    const selected = vessel.length === 1 ? vessel[0] : undefined;
    if (selected) return { runDirectory, path: selected, manifest };
  }

  throw new Error(
    `No ${LAYER_ID} PNG found. Run pnpm comfyui:vessel:generate first or pass --candidate-dir.`,
  );
}
