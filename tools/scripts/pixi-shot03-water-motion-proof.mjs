...
async function main() {
  ...
  try {
    ...
    for (const frame of SAMPLE_FRAMES) {
      ...
      const repeated = await canvas.screenshot({ animations: 'disabled' });
      ...
      await writeFile(repeatedPath, repeated);
      repeatability.push({
        frame,
        pass,
