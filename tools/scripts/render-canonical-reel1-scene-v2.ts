...
async function main() {
  ...
  for (const [index, scenePath] of SCENE_PATHS.entries()) {
    ...
    if (loaded.scene.shots.length !== 1) {
      throw new Error(
        `${scenePath} must contain exactly one shot for canonical Reel 1 assembly.`,
      );
    }
  ...
  for (const scene of loadedScenes.slice(1)) {
    if (
      ...
      throw new Error(
        'All canonical Reel 1 Scene V2 shots must share width, height, and fps.',
      );
    }
  }
  ...
  const propsPath = join(
    outputDirectory,
    'canonical-reel1-scene-v2-props.json',
  );
  const sceneEvidencePath = join(
    outputDirectory,
  ...
  console.log(
    `[ok] duration: ${REEL_DURATION_FRAMES} frames / ${REEL_DURATION_FRAMES / first.fps}s`,
  );
...
function buildCanonicalTimeline(scenes: SceneV2[]) {
  const timeline = scenes.map((scene, index) => {
    ...
    if (shot.sourceStartFrame === undefined) {
      throw new Error(
        `Shot ${shot.sourceShotNumber} is missing sourceStartFrame provenance.`,
      );
    }
    ...
    if (nextStartFrame === undefined) {
      throw new Error(
        `Shot ${shot.sourceShotNumber + 1} is missing sourceStartFrame provenance.`,
      );
    }
...
async function verifyCanonicalManifest(manifestValue: AnimationAssetManifest) {
  if (manifestValue.shots.length < 8) {
    throw new Error(
      'animation-v1 manifest does not contain all eight Reel 1 shots.',
    );
  }
  ...
  for (const shotNumber of Array.from({ length: 8 }, (_, index) => index + 1)) {
    ...
    for (const layerId of requiredIds) {
      ...
      if (
        ...
        throw new Error(
          `Required Shot ${shotNumber} layer ${layerId} is not canonically approved.`,
        );
      }
